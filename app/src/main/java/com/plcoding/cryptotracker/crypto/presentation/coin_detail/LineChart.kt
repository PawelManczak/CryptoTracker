...
fun LineChart(
    ...
    Canvas(modifier = modifier
        ...
        val maxYLabelYWidth = yLabelTextLayoutResults.maxOfOrNull { it.size.width } ?: 0
            ...
            if (showHelperLine) {
                drawLine(
                    color = if (selectedDataPointIndex == index) {
                        ...
                        x = x + textLayoutResult.size.width / 2f, y = viewPortBottomY
                    ), end = Offset(
                        x = x + textLayoutResult.size.width / 2f, y = viewPortTopY
        ...
        yLabelTextLayoutResults.forEachIndexed { index, result ->
            val x = horizontalPaddingPx + maxYLabelYWidth - result.size.width.toFloat()
            val y =
            ...
            drawPoints.forEachIndexed { index, dataPoint ->
                if (isShowingDataPoints) {
                    ...
                    drawCircle(
                        color = style.unselectedColor, center = circleOffset, radius = 10f
                    )

                    if (selectedDataPointIndex == index) {
                        drawCircle(
                            color = Color.White, center = circleOffset, radius = 15f
                        )
                        drawCircle(
                            color = style.selectedColor,
                            center = circleOffset,
...
private fun getSelectedDataPointIndex(
    dataPoints: List<DataPoint>, touchOffsetX: Float, triggerWidth: Float
): Int {
